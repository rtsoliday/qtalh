...
are preserved. The legacy C/C++ runtime sources have not been changed.
...
Outputs are `bin/Linux-<uname -m>/{alh,alh_printer,alh_DB,qtalh,qtalh_printer,qtalh_DB}`.
Qt objects and generated resources are separated into
`qtalh/O.Linux-<architecture>-qt5/` and `qtalh/O.Linux-<architecture>-qt6/`.
Building a different Qt major copies that variant into the common binary directory.
Legacy objects live in `alh/O.Linux-<architecture>/`.

Requirements common to the builds: Linux, GNU Make, C/C++ compilers, Perl,
pkg-config, built EPICS Base with Channel Access/Com headers and libraries,
and TI-RPC development files. QtALH additionally needs Qt **5.15 or newer**, or
...
`LDFLAGS`, `LDLIBS`, `RPC_CFLAGS`, and `RPC_LIBS` can be overridden. Legacy
...
CDEV, CMLOG, and non-Linux support are outside the Qt port's scope.
...
make -C qtalh test-ui            # uses QT_QPA_PLATFORM=offscreen
...
The locally verified environment is Qt 5.15.3, EPICS Base 7.0.8 and Linux x86_64.
Qt 6 is supported by the build selection and source API choices but has **not
been compiled or tested here** because its development packages are unavailable.
...
`qtalh -O`. Mixed legacy/Qt helpers use the same native Linux queue format.
